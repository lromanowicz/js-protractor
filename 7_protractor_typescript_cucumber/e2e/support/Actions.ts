...
class Actions {
    ...
    public pressButton(buttonText: string) {
        return this.click(
            element(by.xpath(`//button[text()="${buttonText}"]`))
        );
    }
    ...
    public getText(element: ElementFinder) {
        return this.waitForElementToBePresent(element)
            .then(() =>
                this.log('Get text from element', this.elementToString(element))
            )
            .then(() => this.highlightElement(element))
    ...
    public waitForElementToBePresent(element: ElementFinder) {
		...
		public waitForText(element: ElementFinder, text: string) {
			return this.log(
					'waitForText',
					text,
					'into',
